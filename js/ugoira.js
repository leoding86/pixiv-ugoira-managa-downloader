(function(common, button){
    ...
    function generateGIF() {
        // 移除点击事件
        button.getBtn('#generate-gif-btn').removeEventListener('click', generateGIF);
        button.getBtn('#generate-gif-btn').innerText = common.lan.msg('generating');
        var zip = new JSZip();

        zip.loadAsync(zipData).then(function (zip) {
            return zip.file(pixivContextData.frames[0].file).async("base64");
        }).then(function (base64) {
            var fimageBase64 = "data:" + pixivContextData.mime_type + ";base64," + base64;

            setCanvasSizeFromImage(fimageBase64, function() {
                var gif = new GIF({
                    workers: 4,
                    quality: 10,
                    width: this.width,
                ...
                appendImagesToGifFrame(gif, zip).then(function () {
                    gif.on('progress', function(p) {
                        button.notice('#generate-gif-btn', Math.round(p * 100) + '%');
                    });
    ...
    function appendImagesToGifFrame(gif, zip, currentIndex) {
        return new Promise(function (resolve) {
            var index = currentIndex || 0;

            if (index < pixivContextData.frames.length) {
                zip.file(pixivContextData.frames[index].file).async('base64').then(function (base64) {
                    var imageBase64 = "data:" + pixivContextData.mime_type + ";base64," + base64;
                    var image = new Image();
                    image.src = imageBase64;
                    gif.addFrame(image, {delay: pixivContextData.frames[index].delay});
                    resolve(appendImagesToGifFrame(gif, zip, ++index));
                });
            } else {
                resolve();
            }
    ...
    function generateWebM() {
        button.getBtn('#generate-webm-btn').removeEventListener('click', generateWebM);
        button.getBtn('#generate-webm-btn').innerText = common.lan.msg('generating');
        var zip = new JSZip();
        zip.loadAsync(zipData).then(function (zip) {
            return zip.file(pixivContextData.frames[0].file).async("base64");
        }).then(function (base64) {
            ...
            encoder.on('progress', function(total, index) {
                button.notice('#generate-webm-btn', Math.round(index * 100 / total) + '%');
            });
            var size = getIllustSize('full');
            appendImagesToWebMFrame(encoder, zip, size).then(function (encoder) {
    ...
    function appendImagesToWebMFrame(encoder, zip, size, currentIndex) {
            ...
            if (index < pixivContextData.frames.length) {
                zip.file(pixivContextData.frames[index].file).async("base64").then(function (base64) {
                    ...
                    getCanvasFromDataURI(imageBase64, size).then(function (canvas, context) {
                        encoder.add(canvas, pixivContextData.frames[index].delay);

                        resolve(appendImagesToWebMFrame(encoder, zip, size, ++index));
                    });
                });
            } else {
                resolve(encoder);
    ...
    if (pixivContext.ugokuIllustFullscreenData !== undefined) {
        pixivContextData = pixivContext.ugokuIllustFullscreenData;
    }
    else if (pixivContext.ugokuIllustData !== undefined) {
        pixivContextData = pixivContext.ugokuIllustData;
    }

    if (pixivContextData !== undefined) {
        if (document.querySelector('.bookmark-container') == null) {
            btnWrapper = document.createElement('div');
            document.querySelector('.userdata').appendChild(btnWrapper);
        } else {
            btnWrapper = document.querySelector('.bookmark-container');
        }

        /* 如果获得图片数据包，添加下载数据包按钮 */
        var btn = button.addBtn(chrome.i18n.getMessage('downloadZipFile'), '', btnWrapper);
        btn.href = pixivContextData.src;
        btn.className = '_bookmark-toggle-button add-bookmark';

        chrome.runtime.sendMessage({from: 'pixivContentScript', to: 'background', step: 'download'});
        ...
        xhr.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                /* 添加合成动图按钮 */
                var btn = button.addBtn(chrome.i18n.getMessage('generate_gif_btn_text'), 'generate-gif-btn', btnWrapper);
                btn.className = '_bookmark-toggle-button add-bookmark';
                btn.addEventListener('click', generateGIF);

                /* 添加合成webm按钮 */
                var btnWebM = button.addBtn(chrome.i18n.getMessage('generate_webm_btn_text'), 'generate-webm-btn', btnWrapper);
                btnWebM.className = '_bookmark-toggle-button add-bookmark';
                btnWebM.addEventListener('click', generateWebM);
            }
            else if (/^(5|4|3)\d\d/.test(this.status)) {
                /* 如果下载数据出错，添加出错按钮 */
                var btn = button.addBtn(chrome.i18n.getMessage('generate_btn_error_text'), '', btnWrapper);
                btn.className = '_bookmark-toggle-button add-bookmark';
            }
        }
        ...
        xhr.open('GET', pixivContextData.src);
        xhr.send();
    }

    function setCanvasSizeFromImage(src, done) {
        var image = new Image();
    ...
    function getIllustSize(type) {
        let size = {};

        if (type === 'full') {
            if (undefined == pixivContext.ugokuIllustFullscreenData.size) {
                size.width  = pixivContext.ugokuIllustData.size[0];
                size.height = pixivContext.ugokuIllustData.size[1];
            } else {
                size.width  = pixivContext.ugokuIllustFullscreenData.size[0];
                size.height = pixivContext.ugokuIllustFullscreenData.size[1];
            }
        } else {
            size.width  = pixivContext.ugokuIllustData.size[0];
            size.height = pixivContext.ugokuIllustData.size[1];
