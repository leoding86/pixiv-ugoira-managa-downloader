(function(common, button){
    ...
    function generateGIF() {
        ...
        var fimageFile = zip.file(pixivContextData.frames[0].file);
        if (fimageFile) {
            var fimageBase64 = "data:" + pixivContextData.mime_type + ";base64," + JSZip.base64.encode(fimageFile.asBinary());
            ...
            setCanvasSizeFromImage(fimageBase64, function() {
                ...
                for (var i = 0; i < pixivContextData.frames.length; i++) {
                    var imageFile = zip.file(pixivContextData.frames[i].file);
                    if (imageFile) {
                        var imageBase64 = "data:" + pixivContextData.mime_type + ";base64," + JSZip.base64.encode(imageFile.asBinary());
                        var image = new Image();
                        image.src = imageBase64;
                        gif.addFrame(image, {delay: pixivContextData.frames[i].delay});
                    }
                }
        ...
        if (fimageFile) {
            ...
            for (var i = 0; i < pixivContextData.frames.length; i++) {
                ...
                if (imageFile) {
                    var imageBase64 = "data:" + pixivContextData.mime_type + ";base64," + JSZip.base64.encode(imageFile.asBinary());
                    (function(index) {
                        getCanvasFromDataURI(imageBase64, size, function(canvas, context) {
                            encoder.add(canvas, pixivContextData.frames[index].delay);
                            // encoder.add(canvas);
                            if (index == pixivContextData.frames.length - 1) {
